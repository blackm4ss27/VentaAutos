...
public class VizualizarAdapter extends BaseAdapter implements Filterable{
    // declarando variables para usarlo
    Context context;                                    //almacenar card view
    ArrayList<VizualizarItems.vizualizarItems> mdatos;      //(llamar datos) el array que se creo en la otra calse , se usa con nombre mdatos
    LayoutInflater inflater;                            //es un fragmente remplzable dentro del activity de manera dinamica
    VizualizarAdapter.CustomerFilter filtro;          //filtra
    ArrayList<VizualizarItems.vizualizarItems> filtroList;  // (filtar datos)

    //constructor para llamar datos
    public VizualizarAdapter(Context context,ArrayList<VizualizarItems.vizualizarItems> datos){
    ...
    private class CustomerFilter extends  Filter{
        ...
        protected Filter.FilterResults performFiltering(CharSequence constraint) {
            ...
            if(constraint != null && constraint.length()>0){
                ...
                ArrayList<VizualizarItems.vizualizarItems> filtro = new ArrayList<VizualizarItems.vizualizarItems>();

                for(Integer i = 0; i<filtroList.size(); i++){
                    if(filtroList.get(i).getTitulo().toUpperCase().contains(constraint)){
                        VizualizarItems.vizualizarItems
                                c= new VizualizarItems.vizualizarItems(
                                filtroList.get(i).getId(),
                                filtroList.get(i).getTitulo(),
                                filtroList.get(i).getImagen(),
                                filtroList.get(i).getTipoCarro());
        ...
        protected void publishResults(CharSequence constraint, Filter.FilterResults results) {
            mdatos = (ArrayList<VizualizarItems.vizualizarItems>) results.values;
